...
"CREATE TABLE IF NOT EXISTS average (param_id INTEGER FOREIGN KEY, N INTEGER, eigs INTEGER, samples INTEGER, chi_av REAL, sd_chi_av_stat REAL, sd_chi_av_rmt REAL, "
"  CONSTRAINT chk_valid_average CHECK (N > 0 AND samples > 0 AND eigs > 0 AND chi_av >= 0.0 AND sd_chi_av_stat >= 0.0 AND sd_chi_av_rmt >= 0.0), "
"  CONSTRAINT chk_unique_average UNIQUE (param_id, N, eigs))"
"CREATE TABLE IF NOT EXISTS quintile (param_id INTEGER FOREIGN KEY, N INTEGER, eigs INTEGER, samples INTEGER, chi_qu REAL, sd_chi_qu_stat REAL, sd_chi_qu_rmt REAL, "
"  CONSTRAINT chk_valid_quintile CHECK (N > 0 AND samples > 0 AND eigs > 0 AND chi_qu >= 0.0 AND sd_chi_qu_stat >= 0.0 AND sd_chi_qu_rmt >= 0.0), "
"  CONSTRAINT chk_unique_quintile UNIQUE (param_id, N, eigs))"
